...
//	cout	<< "SevenEval constructor called" << endl;
  ...
  // Erase all ranks and flush-ranks
  ...
  FiveEval const* five_card_evaluator = new FiveEval();
              ...
              for (int p = 0; p <= n; ++p) {
                if (i != m && j != n && k != p) {
                  ...
                  short unsigned const rank =
                      five_card_evaluator->GetRank(4*i, 4*j, 4*k, 4*l, (4*m)+1,
                          (4*n)+1, (4*p)+1);
                  mRankArray[key < CIRCUMFERENCE_SEVEN ?
            ...
            for(int n = 1; n < m; ++n) {
              for(int p = 0; p < n; ++p) {
                int const key = face_flush[i] + face_flush[j] +
                    ...
                    face_flush[p];
                mFlushRankArray[key] = five_card_evaluator->GetRank(4*i, 4*j,
                        4*k, 4*l, 4*m, 4*n, 4*p);
                ++count;
              }
            }
        ...
        for (int l = 2; l < k; ++l) {
          for (int m = 1; m < l; ++m) {
            for (int n = 0; n < m; ++n) {
              ...
              mFlushRankArray[key] =
                  five_card_evaluator->GetRank(4*i, 4*j, 4*k, 4*l, 4*m, 4*n,
                      51);
              ++count;
            }
          }
      ...
      for (int k = 2; k < j; ++k) {
        for (int l = 1; l < k; ++l) {
          for (int m = 0; m < l; ++m) {
            ...
            mFlushRankArray[key] = five_card_evaluator->GetRank(4*i, 4*j, 4*k,
                4*l, 4*m);
            ++count;
          }
        }
  ...
  // Finished working with the five-card evaluator.
  delete five_card_evaluator;
...
//	cout << "GetRank called with: " << i << "|" << j << "|" << k << "|" << l << "|" << m << "|" << n << "|" << p << endl;
  ...
  if (flush_suit == NOT_A_FLUSH) {
    ...
    return mRankArray[(key < CIRCUMFERENCE_SEVEN ? key :
